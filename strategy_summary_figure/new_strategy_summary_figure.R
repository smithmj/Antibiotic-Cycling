...
pdf(file="strategy_summary_figure/strategy_summary_figure_rev.pdf",family="Times", width = 3.9, height = 3.3,pointsize=9)
...
barpos <- barplot(mat, beside = T, xlim=c(.5,9.5), space=c(0,0.5), col=c("gray22", "gray62"),ylab="final frequency of sensitive genotype", ylim=c(0,.8),names.arg=c("single cycle\nshort period","infinite cycles\nshort period","single cycle\nlong period","infinite cycles\nlong period"), las=1)
